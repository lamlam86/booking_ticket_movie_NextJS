                ...
                {editMode ? (
                  <>
                    <button
                      ...
                      onClick={() => { setEditMode(false); setEditForm({}); setMessage({ type: "", text: "" }); }}
                    >
                      Hủy
                    </button>
                    <div style={{display: "flex", gap: 8}}>
                      {editForm.status === "cancelled" && selectedOrder.status !== "cancelled" && (
                        <button
                          className="btn"
                          style={{background: "var(--danger)"}}
                          onClick={async () => {
                            if (!confirm("Bạn có chắc muốn HỦY đơn hàng này? Ghế sẽ được mở lại.")) return;
                            setSaving(true);
                            try {
